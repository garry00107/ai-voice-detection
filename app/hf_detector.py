...
class HFDetector:
    ...
    def detect(self, audio_bytes: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get top prediction
            top_result = result[0]
            label = top_result.label.lower() if hasattr(top_result, 'label') else str(top_result.get('label', '')).lower()
            score = float(top_result.score if hasattr(top_result, 'score') else top_result.get('score', 0))
